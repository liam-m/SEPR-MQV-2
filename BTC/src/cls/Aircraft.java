...
public class Aircraft {
	...
	/**
	 * Aircraft is landing if it has reached the destination if its destination
	 * is the airport it must also have been given the land command
	 */
	private boolean hasFinished;
	/**
	 * Whether the aircraft is waiting to land. False if the destination is not
	 * the airport. If the destination is the airport, True until land() is
	 * called.
	 */
	...
	/**
	 * Used when calculating when a label representing the score a particular
	 * plane scored should disappear
	 */
	...
	/**
	 * Used outside of Aircraft class to assign a (system) time to a plane that
	 * successfully left airspace
	 * 
	 * @param departureTime
	 *            (system time when a plane departed)
	 */
	 ...
	 */
	private final static audio.Sound WARNING_SOUND = audio.newSoundEffect("sfx"
	...
	/**
	 * This variable increases the multiplierVariable when a plane successfully
	 * leaves the airspace.
	 */
	...
	public Aircraft(String name, String nameDestination, String nameOrigin,
			...
			graphics.Image img, double speed, Waypoint[] sceneWaypoints,
			int difficulty) {
		flightName = name;
		destinationName = nameDestination;
		originName = nameOrigin;
		image = img;
		timeOfCreation = System.currentTimeMillis() / 1000; // System time when
															// aircraft was
															// created in
															// seconds.
		...
		int altitudeOffset = RandomNumber.randInclusiveInt(0, 1) == 0 ? 28000
				: 30000;
		...
		if (isAt(currentTarget)) {
			if (currentTarget.equals(destination)) { // At destination
				if (!is_waiting_to_land) { // Ready to land
					hasFinished = true;
					if (destination.equals(Demo.airport.position())) { // Landed
																		// at
																		// airport
				...
				currentRouteStage++;
				// Next target is the destination if you're at the end of the
				// plan, otherwise it's the next waypoint
				currentTarget = currentRouteStage >= route.length ? destination
						: route[currentRouteStage].position();
			}
		}
	...
	public void draw(int controlAltitude) {
		...
		graphics.draw(image, scale, position.x() - image.width() / 2,
				position.y() - image.height() / 2, getBearing(), 8, 8);
	...
	public void drawFlightPath(boolean is_selected) {
		...
		if (currentTarget != destination) {
			graphics.line(position.x() - image.width() / 2, position.y()
					- image.height() / 2, route[currentRouteStage].position()
					.x(), route[currentRouteStage].position().y());
		}
		...
		if (currentTarget == destination) {
			graphics.line(position.x() - image.width() / 2, position.y()
					- image.height() / 2, destination.x(), destination.y());
		} else {
		...
		while (!atDestination) {
			for (Waypoint point : waypoints) {
				...
				if (skip
						...
						| (point.isEntryOrExit() && !(point.position()
								.equals(destination.position())))) {
	...
	public int updateCollisions(double time_difference,
			ArrayList<Aircraft> aircraftList, Score score) {
		planesTooNear.clear();
		for (int i = 0; i < aircraftList.size(); i++) {
			...
			if (plane != this && isWithin(plane, RADIUS)) {
				...
				score.increaseMeterFill(-1); // Punishment for breaching
												// separation rules (applies to
	 ...
	 * 
	 * @return total distance a plane needs to pass based on its flight plan to
	 *         get to its exit point
	 */
